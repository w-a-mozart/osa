  ...
  {if ($event.is_online_registration) && (!$session || $session->get('userID') <= 0) && (!$open_to_public)}
      <br/>
       <div class="messages help cms_user_help-section">
         ...
         {assign var=regURL value="$loginURL/register"}
      	 {ts 1=$loginURL}Please <a href='%1'>login</a> to register for this event.{/ts}
         {ts 1=$regURL}If you do not have an account on our system, please click <a href='%1'>here</a> to create one.{/ts}
      </div>
  {/if}
