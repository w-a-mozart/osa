...
<div class="crm-event-id-{$event.id} crm-block crm-event-thankyou-form-block">
    ...
    {if isset($volunteer)}
      <div class="volunteer_link-section" style="display: table-cell;">{$volunteer.text}
        <a href="{$volunteer.url}" title="{$volunteer.button_text}" class="button crm-volunteer_signup-button" style="margin-bottom: 20px; margin-left: 20px; float: none; display: inline-block;"><span>{$volunteer.button_text}</span></a>
      </div><br /><br />
    {/if}
