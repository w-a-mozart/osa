...
jQuery(document).ready(function($) {
  ...
  cj("#contact_id").change(function () {
    ...
    CRM.api3('CustomValue', 'get', {
      "entity_id": contact_id
    }).then(function(result) {
      t_contact = result.values;
      if (t_contact) {

        if (typeof t_contact[86] !== 'undefined') {
          var input = cj("#custom_175");
          input.val(t_contact[86].latest);
          input.change();
        }

        if (typeof t_contact[20] !== 'undefined') {
          var input = cj("#custom_174");
          input.val(t_contact[20].latest);
          input.change();
        }
      }
    }, function(error) {
  ...
  // close full items
  cj("#price_474_1730").prop('checked', false).prop('disabled', true);
  cj("label[for='price_474_1730']").html('<span class="crm-price-amount-label" style="text-decoration: line-through;">Private Lesson (30 min) for Book 2+</span> - FULL').css('pointer-events', 'none');
  
  cj("#price_474_1729").prop('checked', false).prop('disabled', true);
  cj("label[for='price_474_1729']").html('<span class="crm-price-amount-label" style="text-decoration: line-through;">Chamber Group Coaching</span> - FULL').css('pointer-events', 'none');
  
  cj("#CIVICRM_QFID_1726_2").prop('checked', false).prop('disabled', true);
  cj("label[for='CIVICRM_QFID_1726_2']").html('<span class="crm-price-amount-label" style="text-decoration: line-through;">Book 2/3 (4 students/session)</span> - FULL').css('pointer-events', 'none');
