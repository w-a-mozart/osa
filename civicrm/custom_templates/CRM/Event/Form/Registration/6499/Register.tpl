...
jQuery(document).ready(function($) {
  ...
  function set_student() {
    cj("input[name='price_469']").prop('disabled', false);
    ...
    CRM.api3('Participant', 'get', {
      ...
      "event_id": {"IN":[5665,5666]},
      "status_id": ["Registered","Pending from pay later"]
    }).then(function(result) {
      if (result.count > 0) {
        cj("input[name='price_469']").prop('disabled', true);
        cj("#CIVICRM_QFID_1703_2").prop('disabled', false).prop('checked', true);
      } else {
        cj("#CIVICRM_QFID_1703_2").prop('checked', false).prop('disabled', true);
      }
    }, function(error) {
      // do nothing
    });
    ...
    CRM.api3('CustomValue', 'get', {
      ...
      if (t_contact) {
        ...
        if (typeof t_contact[1] !== 'undefined') {
          var input = cj("#custom_213");
          input.val(t_contact[1].latest);
          input.change();
        }
