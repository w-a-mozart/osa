  ...
  function buildQuickForm() {
    ...
    foreach ($this->_needs as &$need) {
      ...
      if (!empty($need['start_time'])) {
        $start = new DateTime($need['start_time']);
        $end   = (!empty($need['end_time'])) ? new DateTime($need['end_time']) : $start;
        $start_date = $start->format('M j, Y');
        $end_date   = $end->format('M j, Y');
        if ($start_date == $end_date) {
          $time = (!empty($need['end_time'])) ? $start->format('g:ia') . ' - ' . $end->format('g:ia') : $start->format('g:ia');
          $need['display_time'] = $start_date . ' ' . $time;
        } else {
          $need['display_time'] = $start_date . ' - ' . $end_date;
        }
      } else {
        $need['display_time'] = '';
      }
      if (!empty($need['description'])) {
        $need['role_description'] .= (empty($need['role_description'])) ? $need['description'] : ' - ' . $need['description'];
      }
    }
...
//      $activityValues['time_scheduled_minutes'] = CRM_Utils_Array::value('duration', $need);
      $activityValues['credits'] = CRM_Utils_Array::value('duration', $need);
      ...
      if (!empty($need['role_description'])) {
        $tmp_descrip = (!empty($need['description'])) ? ' - ' . $need['description'] : '';
        $need['description'] = $need['role_description'] . $tmp_descrip;
      }
