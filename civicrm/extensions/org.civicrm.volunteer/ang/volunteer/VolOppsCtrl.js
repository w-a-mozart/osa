(function (angular, $, _) {
  ...
  angular.module('volunteer').controller('VolOppsCtrl', function ($route, $scope, $window, $timeout, crmStatus, crmUiHelp, volOppSearch, countries, settings, supporting_data) {
    ...
    // volOppSearch.search();
    $scope.searchMessage = ts('Searching for available volunteer opportunities...');
    volOppSearch.search().then(function(result) {
      $scope.searchMessage = ts('No results found. Please try different search criteria.');
      // not sure how else to tell that the first search/load is finished
      $scope.afterFirstLoad();
    });
