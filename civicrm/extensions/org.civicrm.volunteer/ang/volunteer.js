    ...
    .factory('volOppSearch', ['crmApi', '$location', '$route', function(crmApi, $location, $route) {
      ...
      volOppSearch.search = function() {
        ...
        return crmApi('VolunteerNeed', 'getsearchresult', volOppSearch.params).then(function(data) {
          result = _.map(data.values, function(need) {
            var start = new Date(need.start_time.replace(' ', 'T'));
            var end   = new Date(need.end_time.replace(' ', 'T'));

            need.short_date = start.toLocaleDateString('en-CA', {month: 'short', day: 'numeric'});
            var end_short_date = end.toLocaleDateString('en-CA', {month: 'short', day: 'numeric'});
            
            if (need.short_date == end_short_date) {
              var start_time = start.toLocaleTimeString('en-CA', {hour: 'numeric', minute: 'numeric'});
              var timespan = ' ' + start_time + ' - ' + end.toLocaleTimeString('en-CA', {hour: 'numeric', minute: 'numeric'});
              need.short_date += ' ' + timespan;
              need.long_date = start.toLocaleDateString('en-CA', {month: 'short', day: 'numeric', year: 'numeric'}) + timespan;
            } else {
              need.short_date += ' - ' + end_short_date;
              need.long_date = start.toLocaleDateString('en-CA', {month: 'short', day: 'numeric', year: 'numeric'}) +
