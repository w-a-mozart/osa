...
");
  ...
  public static function relationshipList($contact_ids, $type) {
    ...
    $contactID = CRM_Core_Session::getLoggedInContactID();
    if (empty($contactID)) {
      $uid = (is_object($GLOBALS['user'])) ? $GLOBALS['user']->uid : 0;
      $result = civicrm_api('UFMatch', 'get', array('uf_id' => $uid, 'sequential' => 1, 'version' => 3));
      $contactID = isset($result['values'][0]['contact_id']) ? $result['values'][0]['contact_id'] : NULL;
      if (empty($contactID)) {
        return array();
      }
      CRM_Core_Error::debug_log_message('Need to keep custom_php\CRM\Contact\BAO\Contact\Permission.php');
    }
