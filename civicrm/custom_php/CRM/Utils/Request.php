...
class CRM_Utils_Request {
  ...
  public static function retrieve($name, $type, &$store = NULL, $abort = FALSE, $default = NULL, $method = 'REQUEST', $isThrowException = FALSE) {
    ...
    if (!isset($value) && $abort) {
      // hack for refresh of a form which is now missing the query string
      // try to get it from the REFERER
      $referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
      list($script_url, $ignore) = explode('?', $GLOBALS['base_root'] . $_SERVER['REQUEST_URI'], 2);
      list($ref_url, $ref_qry) = explode('?', $referer, 2);

      if (strcmp($script_url, $ref_url) !== FALSE) {
        $arr = array();
        parse_str($ref_qry, $arr);
        $value = CRM_Utils_Array::value($name, $arr);
      }
      if (!isset($value)) {
        if ($isThrowException) {
