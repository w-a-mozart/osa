...
//    self::invalidKeyCommon();
  ...
  public function invalidKeyRedirect() {
    if (($_POST['form_id'] == 'user_login_block') && !empty($_GET['id']) && !empty($_GET['q'])) {
      $url = $GLOBALS['base_url'] . '/' . $_GET['q'] . '?id=' . $_GET['id'];
      unset($_GET['q']);
      unset($_GET['id']);
    ...
    elseif (!empty($this->_entryURL) && $url_parts = parse_url($this->_entryURL)) {
      ...
      if (!empty($url_parts['path'])) {
        ...
        CRM_Core_Session::setStatus(ts('Your browser session has been reset, so we have returned you to the beginning of the process. If you continue to experience difficulties, please contact us for assistance.'));
        return CRM_Utils_System::redirect($redirect_url);
      }
    }

    CRM_Utils_System::setUFMessage(ts('An unexpected error has occurred. We have reset your browser session so that you can retry your request. If you continue to experience difficulties, please contact us for assistance.'));
    return CRM_Utils_System::redirect($GLOBALS['base_url']);
