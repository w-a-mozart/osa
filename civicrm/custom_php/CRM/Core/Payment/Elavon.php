...
class CRM_Core_Payment_Elavon extends CRM_Core_Payment {
  ...
  function mapProcessorFieldstoParams($params) {
    ...
    $requestFields['ssl_cvv2cvc2_indicator'] = isset($params['cvv2']) ? "1" : NULL;
...
EOD;
    ...
    if ($processorResponse['ssl_result_message'] != "APPROVAL" && $processorResponse['ssl_result_message'] != 'SUCCESS') {
  ...
  function buildXML($requestFields) {
    ...
    foreach ($requestFields as $key => $value) {
      if (isset($value)) {
        $xml .= '<' . $key . '>' . self::tidyStringforXML($key, $value) . '</' . $key . '>';
      }
    }
    $xml .= '</txn>';
  ...
  // the xml is posted to a url so must not contain spaces etc.
  function tidyStringforXML($key, $value) {
    ...
    static $xmlFieldLength = array();
    
    if (empty($xmlFieldLength)) {
    ...
    $fieldlength = isset($xmlFieldLength[$key]) ? $xmlFieldLength[$key] : 255;
