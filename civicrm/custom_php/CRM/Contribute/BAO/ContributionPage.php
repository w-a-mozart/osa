...
class CRM_Contribute_BAO_ContributionPage extends CRM_Contribute_DAO_ContributionPage {
  ...
  static function sendMail($contactID, &$values, $isTest = FALSE, $returnMessageText = FALSE, $fieldTypes = NULL) {
    ...
    if ( CRM_Utils_Array::value('is_email_receipt', $values) ||
      ...
      // we always want these emails to go to the parent
      // can't do this in osa_civicrm_alterMailParams because
      // CRM_Utils_Hook::alterMailParams doesn't get called if $email = NULL
      module_load_include('inc', 'osa');
      $to_contact = _osa_get_contact_id();  // logged in user
      if ($to_contact != $contactID) {
        $to_contact = array_key_exists($to_contact, _osa_getHouseholdMembers($contactID, 'display_name')) ? $to_contact : _osa_getHousehold($contactID);
      }
      list($displayName, $email) = CRM_Contact_BAO_Contact_Location::getEmailDetails($to_contact);
