{*
 ...
 +--------------------------------------------------------------------+
 | Override Standard CiviCRM Template for Groups                      |
 | - changes to text                                                  |
 +--------------------------------------------------------------------+
...
{if $groupCount eq 0 }
    <div class="messages status">
      ...
      {ts}You are not currently subscribed to any mailing lists.{/ts}
    </div>
{/if}
{if $groupIn }  
    <div class="form-item">
...
{if $groupPending }
    <div class="form-item">
      ...
      <div class="description">{ts}Your subscription to these lists is pending confirmation.{/ts}</div>		  
      <div>
  {strip}
        <table class="selector">
          <tr class="columnheader">
...
{if $groupOut }
    <div class="form-item">
      <div class="label status-removed">{ts}Unsubscribed Lists{/ts}</div>
      <div class="description">{ts}You are no longer subscribed to these lists. Click Rejoin if you want to re-subscribe.{/ts}</div>	
      <div>
  {strip}
        <table class="selector">
          <tr class="columnheader">
  ...
  {foreach from=$groupOut item=row}
    ...
    {if $edit}
            <td><a href="{crmURL p='civicrm/user/group' q="gcid=`$row.id`&action=delete&st=i"}" onclick ="return confirm('{ts 1=$row.title}Are you sure you want to add back into %1?{/ts}');">[ {ts}Rejoin{/ts} ]</a></td>
    {/if}
          </tr>
  {/foreach}
        </table>
